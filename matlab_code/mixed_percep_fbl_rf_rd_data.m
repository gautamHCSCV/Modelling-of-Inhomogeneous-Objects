...
imp = [2,8,27,32,33,97,104,105,109,116,131,136,143,148,154,158,164,170,186,199];
%imp = [2];
...
for im = 1:length(imp)
    ...
    in = weber_sampling(f1,per);
    B1 = zeros( length(dis1), length(order));
    for i = 1:length(order)
         yd = fderiv( dis1,order(i), 180,h );
         B1(:,i) = reshape(yd,[length(yd),1]);
    end 
    ...
    t1 = num2str(j);
    t2 = '_mixed_percep_force_ecoflex_181221_data.csv';
    t3 = '_mixed_percep_pos_ecoflex_181221_data.csv';
