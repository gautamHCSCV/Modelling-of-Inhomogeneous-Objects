...
P = 'C:\Users\DELL\Downloads\Inhomogeneous data\Ecoflex_inhomogeneous_Sinusoidal_181221_position_3000';
...
imp1 = [1,3,4,5,7,9,10,11,12,13,14,15];
imp2 = [2,6,8];
...
for j = 1:1
    ...
    for i = 1:length(imp1)
        ...
        t = f(phi,1:l);
        m = 'C:\Users\DELL\Downloads\Inhomogeneous data\Ecoflex_inhomogeneous_Sinusoidal_181221_position_3000\';
        y = append(m,t);
        Spill1 = importdata(y);
        x = Spill1(:,4);
        y = Spill1(:,16);
        ...
        P_z = P_z - mean(P_z(start+100:start+300));
        
        MF_z = MF_z - mean(MF_z(start+100:start+300));
        
        n = 25;
        B = 1/n*ones(n,1);
        mul = 2.27906977;
        
        P_z = filtfilt(B,1,P_z);
        MF_z = filtfilt(B,1,MF_z) * mul;
        
        x = P_z;
        y = MF_z;

        in = weber_sampling(y,per);
        ...
        for pp = 1:length(order)
          yd = fderiv( x,order(pp), 180,h );
          B(:,pp) = reshape(yd,[length(yd),1]);
        end 
    ...
    t1 = num2str(j);
    t2 = '_train_per_force_ecoflex_181221_derivative_data.csv';
    t3 = '_train_per_pos_ecoflex_181221_derivative_data.csv';
    ...
    for i = 1:length(imp2)
        ...
        in_t = weber_sampling(y,per);


        B1_t = zeros( length(x), length(order));
        for kj = 1:length(order)
             yd = fderiv( x,order(kj), 180,h );
             B1_t(:,kj) = reshape(yd,[length(yd),1]);
        end 

        for mm = 1:length(in_t)
            f1_test = [f1_test;y(in_t(mm),:)];
            
        end
        for mm = 1:length(in_t)
            dis1_test = [dis1_test;B1_t(in_t(mm),:)];
    ...
    t1 = num2str(j);
    t2 = '_test_per_force_ecoflex_181221_derivative_data.csv';
    t3 = '_test_per_pos_ecoflex_181221_derivative_data.csv';
