...
for j = 1:200
    ...
    gk = fft(f1);
    g0k = gk(1)/length(f1);
    g1k = 2*real(gk(2))/length(f1);
    g2k = -2*imag(gk(2))/length(f1);
    g3k = 2*real(gk(3))/length(f1);
    g4k = -2*imag(gk(3))/length(f1);
    g5k = 2*real(gk(4))/length(f1);
    g6k = -2*imag(gk(4))/length(f1);
    g7k = 2*real(gk(5))/length(f1);
    g8k = -2*imag(gk(5))/length(f1);
    g9k = 2*real(gk(6))/length(f1);
    g10k = -2*imag(gk(6))/length(f1);
...
phii = (phi - min(phi))/(max(phi) - min(phi));
Ki = (K - min(K))/(max(K) - min(K));



D = horzcat(ampi,phii,Ki);


csvwrite('fbl_ecoflex_out_1.csv', g0);
csvwrite('fbl_ecoflex_inp_1.csv', D);
