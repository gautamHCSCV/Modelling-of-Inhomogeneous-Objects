...
for j = 1:1
    ...
    for i = 1:length(imp1)
        ...
        B1 = zeros( length(x), length(order));
        for np = 1:length(order)
             yd = fderiv( x,order(np), 180,h );
             B1(:,np) = reshape(yd,[length(yd),1]);
        end 
    ...
    t1 = num2str(j);
    t2 = '_train_force_ecoflex_181221_derivative_data.csv';
    t3 = '_train_pos_ecoflex_181221_derivative_data.csv';
    ...
    t1 = num2str(j);
    t2 = '_test_force_ecoflex_181221_derivative_data.csv';
    t3 = '_test_pos_ecoflex_181221_derivative_data.csv';
